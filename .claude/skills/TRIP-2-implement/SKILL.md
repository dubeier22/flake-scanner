...
You are now in **implementation mode** for **flake-scanner**.
...
```bash
ruff check . 2>&1 | tee /tmp/_trip2-lint.txt
mypy . 2>&1 | tee /tmp/_trip2-typecheck.txt
```

### 2. Run affected unit tests

```bash
pytest <pattern-for-affected-files>
```
...
If the change modifies an externally observable contract — the `calibration_data.csv` schema, the annotated-image/`candidates.csv` output format, or CLI argument shapes — run the affected mode (`calibrate`, `scan`, or `both`) end-to-end against a small fixture image and confirm output looks correct. Docs-only changes skip this.
